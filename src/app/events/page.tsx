      ...
      <section className="py-12 bg-white relative -mt-16 z-20">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center p-6 rounded-2xl bg-white shadow-lg hover:shadow-2xl transition-all">
              ...
              <div className="text-4xl mb-2 font-bold bg-gradient-to-r from-[#1E4AB8] to-[#8B5CF6] bg-clip-text text-transparent">
                {events.length}
              </div>
            ...
            <div className="text-center p-6 rounded-2xl bg-white shadow-lg hover:shadow-2xl transition-all">
              ...
              <div className="text-4xl mb-2 font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
            ...
            <div className="text-center p-6 rounded-2xl bg-white shadow-lg hover:shadow-2xl transition-all">
              ...
              <div className="text-4xl mb-2 font-bold bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">
                {categories.length - 1}
              </div>
            ...
            <div className="text-center p-6 rounded-2xl bg-white shadow-lg hover:shadow-2xl transition-all">
              ...
              <div className="text-4xl mb-2 font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                5
              </div>
      ...
      <section className="py-16 bg-gradient-to-br from-purple-50 to-blue-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full text-sm mb-4 shadow-sm">
              <Sparkles className="w-4 h-4 text-purple-600" />
              <span className="text-purple-700">Event Terdekat</span>
            </div>
            <h2 className="text-3xl font-bold text-gray-800 mb-4">Jangan Lewatkan!</h2>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {upcomingEvents.map((event, index) => (
              <div 
                key={event.id}
                className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-2 cursor-pointer"
              ...
              >
                ...
                <div className="p-6">
                  <h3 className="text-lg font-bold mb-3 line-clamp-2 text-gray-800 group-hover:text-blue-600 transition-colors">{event.title}</h3>
          ...
          {viewMode === 'list' ? (
            ...
            <div className="space-y-6">
              {filteredEvents.map((event) => (
                <div 
                  key={event.id}
                  className="group bg-gradient-to-br from-white to-gray-50 rounded-3xl p-6 md:p-8 shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 cursor-pointer border border-gray-100"
                  onClick={() => setSelectedEvent(event)}
                >
                  <div className="grid md:grid-cols-[200px_1fr] gap-6">
                    ...
                    <div>
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-2xl font-bold mb-2 group-hover:text-[#1E4AB8] transition-colors text-gray-800">
                            {event.title}
                          </h3>
                      ...
                      <div className="grid md:grid-cols-2 gap-4 mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="font-medium text-gray-800">{new Date(event.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                          </div>
                        </div>
                      ...
                      <button 
                        className="px-6 py-2 rounded-xl text-white transition-all hover:opacity-90"
                        style={{ backgroundColor: event.accentColor }}
                      >
                        Lihat Detail
                      </button>
            ...
            <div className="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-2xl font-bold text-gray-800">{monthNames[selectedMonth]} {selectedYear}</h3>
                <div className="flex gap-2">
                  <button 
              ...
              <div className="grid grid-cols-7 gap-2">
                ...
                {Array.from({ length: getDaysInMonth(selectedMonth, selectedYear) }).map((_, i) => {
                  ...
                  return (
                    <div 
                      key={day}
                      className={`aspect-square p-2 rounded-xl border transition-all cursor-pointer flex flex-col items-center justify-start ${
                        isToday ? 'bg-[#1E4AB8] text-white border-[#1E4AB8]' : 'bg-gray-50 border-gray-200 hover:border-[#1E4AB8] hover:shadow-md'
                      }`}
                      onClick={() => {
                    ...
                    >
                      <div className={`text-sm mb-1 font-medium ${isToday ? 'text-white' : 'text-gray-700'}`}>{day}</div>
                      {dayEvents.length > 0 && (
                        <div className="w-full space-y-1 mt-1">
                          ...
                          {dayEvents.length > 2 && (
                            <div className="text-[10px] opacity-70 text-center">+{dayEvents.length - 2}</div>
                          )}
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-scale-up">
            <div className="relative h-64">
              ...
              <button 
                ...
                className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/20 backdrop-blur-md hover:bg-white/40 flex items-center justify-center text-white transition-all"
              >
                <X className="w-6 h-6" />
              </button>
              <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/80 to-transparent">
                <span 
                  className="px-3 py-1 rounded-full text-xs text-white mb-3 inline-block"
            ...
            <div className="p-8">
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                <div className="space-y-4">
                  <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <span className="font-medium">{new Date(selectedEvent.date).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
                  </div>
                  <div className="flex items-center gap-3 text-gray-700">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 text-gray-700">
                    ...
                    <span className="font-medium">{selectedEvent.unit}</span>
                  </div>
                </div>
              </div>
              ...
              {selectedEvent.agenda && selectedEvent.agenda.length > 0 && (
                <div className="mb-8">
                  <h3 className="text-lg font-bold text-gray-800 mb-4">Agenda Kegiatan</h3>
              ...
              <div className="flex gap-4 pt-4 border-t border-gray-100">
                <button 
                  className="flex-1 py-3 px-6 rounded-xl text-white font-medium hover:opacity-90 transition-all flex items-center justify-center gap-2"
                  style={{ backgroundColor: selectedEvent.accentColor }}
                >
                  <CalendarIcon className="w-5 h-5" />
                  <span>Tambahkan ke Kalender</span>
                </button>
                <button className="px-6 py-3 rounded-xl border border-gray-200 hover:bg-gray-50 transition-all flex items-center gap-2 text-gray-700 font-medium">
