...
const CHANNEL_NAME = 'teacher_portal';
...
export default function TeacherMessagesPage() {
  ...
  const [composeTo, setComposeTo] = React.useState('Admin TU');
  ...
  const persistLocal = React.useCallback((nextInbox: InboxMessage[], nextOutbox: OutboxMessage[]) => {
  ...
  const simulateIncoming = () => {
    const id = `tmsg_in_${Date.now()}`;
    const createdAt = nowIso();
    const msg: InboxMessage = {
      id,
      from: 'Admin Sekolah',
      subject: 'Informasi Akademik',
      content: 'Mohon lengkapi input nilai sebelum batas waktu yang ditentukan.',
      date: createdAt,
      read: false,
    };
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <Sidebar
        ...
        panelSubtitle={`${teacher.unit} • ${teacher.subject}`}
      />
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Inbox/Outbox untuk {teacher.name} ({teacher.subject})
              </p>
            </div>
                    ...
                    filteredInbox.map((m) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          {!m.read ? (
                            <button onClick={() => markRead(m.id)} className="mt-3 inline-flex items-center gap-2 text-sm text-[#1E4AB8] hover:underline">
